...
### Programmatic clean → semantic layer → chat flow

Need a ready-to-run snippet that copies the same workflow showcased in the notebooks? The following code imports the `TransformationManager`, cleans the `sales_semantic` DataFrame, persists it as a semantic layer, and finally chats with the freshly loaded dataset:
